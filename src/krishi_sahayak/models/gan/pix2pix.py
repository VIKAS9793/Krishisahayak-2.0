...
# Import canonical schemas from other packages
from krishi_sahayak.config.schemas import OptimizerConfig, SchedulerConfig
...
class DataConfig(BaseModel):
    ...
    mean: List[float] = Field(default=[0.5, 0.5, 0.5])
    std: List[float] = Field(default=[0.5, 0.5, 0.5])
...
class Pix2PixGAN(pl.LightningModule):
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int, optimizer_idx: int) -> torch.Tensor | None:
    ...
    def _log_image_grid(self, real_a: torch.Tensor, real_b: torch.Tensor, fake_b: torch.Tensor) -> None:
        ...
        if isinstance(self.logger, WandbLogger):
            ...
            self.logger.experiment.add_image(f"val_generated_images/Epoch_{self.current_epoch}", grid, self.current_epoch)
    ...
    def configure_optimizers(self) -> List[Dict[str, Any]]:
        ...
        scheduler_conf = self.config.scheduler
        if not scheduler_conf.use_scheduler:
            return [{"optimizer": opt_g}, {"optimizer": opt_d}]
        
        schedulers = []
        for opt in [opt_g, opt_d]:
            if scheduler_conf.scheduler_type == "plateau":
                scheduler = lr_scheduler.ReduceLROnPlateau(opt, mode='min', **scheduler_conf.scheduler_params)
            elif scheduler_conf.scheduler_type == "cosine":
                if "T_max" not in scheduler_conf.scheduler_params:
                    raise ValueError("`T_max` must be provided in scheduler_params for CosineAnnealingLR.")
                scheduler = lr_scheduler.CosineAnnealingLR(opt, **scheduler_conf.scheduler_params)
            else:
                raise ValueError(f"Unsupported scheduler type: {scheduler_conf.scheduler_type}")
            
            schedulers.append({"scheduler": scheduler, "monitor": scheduler_conf.scheduler_monitor})
